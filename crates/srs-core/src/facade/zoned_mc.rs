...
//! [`crate::facade::uncertainty`]. petekStatic's whole-model driver
...
//! [`srs_model::RealizationDraw`] per draw carrying a [`srs_model::ZoneDraw`] per
//! zone (per-zone contact draws + per-zone property levels), realizes each on a
//! stack template ([`crate::facade::model::Model::stack_template`]), reads
//! [`srs_model::StaticModel::in_place_by_zone`], and rolls the per-draw per-zone
...
//! stream in a fixed field order (mirroring [`srs_model::McInputs::realize`]), and
...
use crate::facade::model::{zone_volume, Model};
use crate::facade::spec::DistSpec;
use crate::facade::uncertainty::PSummary;
...
use srs_wireframe::{Contact, ContactKind};
use std::ops::Range;
...
fn zone_base_priors(
    zone_priors: &[(String, srs_model::ConstantPriors)],
    global: srs_model::ConstantPriors,
    name: &str,
) -> srs_model::ConstantPriors {
    zone_priors
        .iter()
        .find(|(z, _)| z == name)
        .map(|(_, p)| *p)
...
mod tests {
    ...
    use super::*;
    use crate::facade::grid::{Frame, StaticGrid};
    ...
    use srs_wireframe::{Boundary, Contact, ContactKind, GriddedDepth, Hardness};
