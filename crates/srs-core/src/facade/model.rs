...
//! MC over a regenerated template (see [`crate::facade::uncertainty`]).

use crate::facade::uncertainty::{run_uncertainty, McConfig, McOutcome};
use crate::facade::zoned_mc::{run_zoned_uncertainty, ZonedMcConfig, ZonedMcOutcome};
use crate::inplace::Fluid;
use crate::view::{IntersectionBundle, MapBundle, MapSpec, SectionSpec, VolumeBundle};
...
/// [`srs_model::Provenance::well_ties`] (the engine `with_well_ties` seam).
#[derive(Debug, Clone)]
...
impl Model {
    ...
    /// [`srs_model::Provenance::well_ties`]. Empty when no tie table was supplied.
