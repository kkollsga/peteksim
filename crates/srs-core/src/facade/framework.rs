...
//!
//! The seam (`srs_model::StaticModelBuilder::from_wireframe`) owns the gridding
//! and warm-start solve; this module only assembles its inputs and computes
//! residuals against the raw horizon surfaces.

use crate::facade::grid::{self, Frame, StaticGrid};
use crate::facade::project::Project;
use crate::facade::spec::LayersSpec;
use crate::facade::zonation::{self, ZoneSpec};
...
use srs_wireframe::{Boundary, GriddedDepth, Hardness, Horizon, HorizonRole, Wireframe};
...
pub struct Framework {
    ...
    /// [`srs_model::StaticModelBuilder::with_zone_priors`]. By zone name; a zone
    ...
    /// [`srs_model::StaticModelBuilder::with_well_ties`] on the stack build — the
    ...
    /// ([`srs_model::StaticModelBuilder::with_collapse_below_m`]): sub-threshold
    ...
    /// ([`srs_model::StaticModelBuilder::with_min_thickness_m`]): where the gridded
...
impl Framework {
    ...
    /// layering + per-zone volumetrics ([`crate::facade::Model::in_place_by_zone`]).
    ...
    /// — assembling the [`srs_model::HorizonStack`] from the resolved sources and
